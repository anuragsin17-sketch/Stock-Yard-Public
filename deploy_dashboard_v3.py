#!/usr/bin/env python3
"""
Deploy Dashboard to EC2 - Using user data approach
"""
...
def deploy_dashboard_v3(config_path='d:\\Stock Yard\\aws_config.json'):
    """Deploy dashboard using EC2 stop/start or by creating new instance"""
    ...
    print("\n" + "="*60)
    print("DEPLOYING DASHBOARD TO EC2 - SIMPLIFIED METHOD")
    print("="*60 + "\n")
    ...
    # Simple deployment using basic shell commands
    print("⏳ Deploying dashboard...\n")
    
    # Create HTML file locally first
...
<body>
    ...
    # Run individual simple commands
    commands = [
        "sudo apt-get update -qq",
        "sudo apt-get install -y nginx",
        "sudo mkdir -p /var/www/stockyard",
        "sudo chown ubuntu:ubuntu /var/www/stockyard"
    ]
    ...
    try:
        ...
        print(f"⚠️  Issue with setup: {e}")
    
    # Copy dashboard content
    print("⏳ Deploying dashboard HTML...")
    try:
        # Create HTML file path
        html_path = "d:\\Stock Yard\\dashboard_temp.html"
        with open(html_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        print("✓ Dashboard created locally")
        
        # Copy to EC2 using SSM
        import subprocess
        
        # Use EC2 to pull the file from a known location
        # For now, just paste directly
        paste_command = f'''echo '{html_content[:1000]}...' > /var/www/stockyard/index.html'''
        
        response = ssm_client.send_command(
            InstanceIds=[instance_id],
            DocumentName="AWS-RunShellScript",
            Parameters={'command': ["cat > /var/www/stockyard/index.html << 'EOF'\n" + html_content + "\nEOF"]},
            TimeoutSeconds=120
        )
        ...
        for _ in range(60):
            ...
            try:
                ...
                if result['Status'] == 'Success':
                    print("✓ Dashboard file copied")
                    break
                elif result['Status'] == 'Failed':
                    print("⚠️  Failed to copy dashboard")
                    break
            except:
                pass
        
    except Exception as e:
        print(f"⚠️  Could not deploy HTML: {e}")
    
    # Configure and start Nginx
    print("⏳ Configuring Nginx...")
    try:
        config_cmd = '''sudo tee /etc/nginx/sites-available/stockyard > /dev/null << EOF
server {
...
sudo systemctl restart nginx'''
        ...
        print(f"⚠️  Nginx config issue: {e}")
    
    # Final success message
    print("\n" + "="*60)
    print("✓ DASHBOARD DEPLOYMENT INITIATED")
    print("="*60)
    print(f"\n🌐 Access dashboard at:")
    ...
    print("✅ Dashboard should be LIVE shortly!")
    print("⏱️  If not accessible, wait 30 seconds and refresh\n")
