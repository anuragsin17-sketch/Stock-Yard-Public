...
"""
    ...
    try:
        ...
        # AMI IDs for different regions (Ubuntu 20.04 LTS)
        ami_map = {
            'us-east-1': 'ami-0c55b159cbfafe1f0',
            'ap-south-1': 'ami-0287a05f0ef0e9d9a',
            'ap-southeast-1': 'ami-0dc5785603ad4ff54'
        }
        ami_id = ami_map.get(config['AWS_REGION'], ami_map['us-east-1'])
        ...
        response = ec2_client.run_instances(
            ImageId=ami_id,  # Ubuntu 20.04 LTS for the region
            MinCount=1,
            MaxCount=1,
            InstanceType='t2.micro',  # t2.micro - free tier eligible
...
def print_summary(instance_info):
    ...
    print(f"  Region: {instance_info.get('region', 'ap-south-1')}")
    ...
    print(f"\n⏰ Cost:")
    print(f"  t2.micro: FREE (12 months free tier)")
...
def main():
    ...
    instance_info['region'] = config['AWS_REGION']
