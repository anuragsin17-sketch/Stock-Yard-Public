#!/usr/bin/env python3
"""
Fix EC2 SSM role and deploy dashboard
"""
...
def fix_and_deploy(config_path='d:\\Stock Yard\\aws_config.json'):
    """Fix SSM role on EC2 and deploy dashboard"""
    ...
    print("\n" + "="*60)
    print("FIXING EC2 SSM ROLE & DEPLOYING DASHBOARD")
    print("="*60 + "\n")
    ...
    # Check/create IAM role for SSM
    print("\n⏳ Checking IAM role for Systems Manager...")
    try:
        ...
        try:
            ...
            print(f"✓ Created IAM role and attached SSM policy")
        ...
        try:
            ...
            print("⏳ Waiting 30 seconds for role to be active...")
            time.sleep(30)
        except Exception as e:
            if "InvalidParameterValue.Duplicate" in str(e) or "already associated" in str(e):
                print("✓ Role already attached to instance")
            else:
                print(f"⚠️  Warning: {e}")
    
    except Exception as e:
        print(f"⚠️  Error setting up IAM role: {e}")
        print("Continuing anyway...\n")
    
    # Deploy dashboard
    print("⏳ Deploying dashboard via Systems Manager...")
    
    deployment_commands = """#!/bin/bash
echo "=========================================="
...
sudo cp /tmp/index.html /var/www/stockyard/index.html
sudo chown www-data:www-data /var/www/stockyard/index.html
echo "✓ Dashboard created"

# Configure Nginx
echo "⏳ Configuring Nginx..."
sudo tee /etc/nginx/sites-available/stockyard > /dev/null << 'CONFEOF'
server {
...
"""
    ...
    try:
        ...
        print("⏳ Waiting for deployment (2-3 minutes)...\n")
        ...
        for attempt in range(90):
            ...
            try:
                ...
                if status == 'Success':
                    ...
                    error = cmd_response.get('StandardErrorContent', '')
                    if output:
                        print(f"\nOutput:\n{output}")
                    if error:
                        print(f"\nError:\n{error}")
                    ...
                    print(f"  Progress: {attempt+1}/90", end='\r')
                    
            except Exception as e:
                print(f"  Checking status... ({attempt+1}/90)", end='\r')
